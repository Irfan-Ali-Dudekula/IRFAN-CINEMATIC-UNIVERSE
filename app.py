...
if not st.session_state.logged_in:
    ...
    if movies_list:
        ...
        for i in range(limit):
            ...
            # Ensure poster and details exist
            ...
            with cols[i % 3]:
                ...
                st.markdown(f"<h3>{getattr(movie, 'title', 'Unknown Title')}</h3>", unsafe_allow_html=True)
                ...
                if details['ott_l']: 
                    st.markdown(f'<a href="{details["ott_l"]}" target="_blank" class="play-button">WATCH NOW</a>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
    else:
        st.warning("The Vault is searching... Please select a Mood and Language.")
